...
export default function StorefrontClient() {
  ...
  useEffect(() => {
    const restoreTimer = window.setTimeout(() => {
      ...
      } catch { localStorage.removeItem("maison-jiya-cart-v2"); }
    ...
    void (async () => {
      ...
      } catch (caught) { setError(caught instanceof Error ? caught.message : "Catalogue indisponible."); }
  ...
  function updateQuantity(key: string, quantity: number) {
    setCart((current) => {
      if (quantity <= 0) { const next = { ...current }; delete next[key]; return next; }
      return { ...current, [key]: Math.min(20, quantity) };
    });
  }
  function beginCheckout() {
    if (!cartLines.length) return;
    setCartOpen(false); setCheckoutOpen(true); setSubmitError("");
  ...
  async function submitOrder(event: FormEvent<HTMLFormElement>) {
    ...
    } catch (caught) { setSubmitError(caught instanceof Error ? caught.message : "Commande impossible."); }
  ...
  return <main className="storefront-shell">
    ...
    <header className="storefront-header">
      <a className="storefront-brand" href="/boutique" aria-label={`${brand} boutique`}>
        {/* eslint-disable-next-line @next/next/no-img-element */}<img src={catalog?.logoUrl || "/maison-jiya-logo.jpeg"} alt={brand} />
        <span><strong>{brand}</strong><small>Boutique officielle</small></span>
      </a>
      <nav><a href="#catalogue">Catalogue</a>{Boolean(catalog?.offers.length) && <a href="#offres">Offres</a>}<a href="#commande">Comment commander</a></nav>
    ...
    <section className={`storefront-hero ${catalog?.heroImageUrl ? "has-image" : ""}`}>
      ...
      {catalog?.heroImageUrl ? <div className="storefront-hero-photo">{/* eslint-disable-next-line @next/next/no-img-element */}<img src={catalog.heroImageUrl} alt="Collection Maison Jiya" /></div> : <div className="storefront-hero-card" aria-hidden="true"><span>MJ</span><strong>{brand}</strong><small>Montres · Bijoux · Wallets · Électronique</small></div>}
    </section>

    {Boolean(catalog?.offers.length) && <section className="storefront-featured-offers" id="offres"><div className="storefront-section-head"><div><span>Offres Maison Jiya</span><h2>Packs & bons plans</h2></div><strong>{catalog?.offers.length} offre(s)</strong></div><div className="storefront-product-grid storefront-offer-grid">{catalog?.offers.map((item) => <StoreItemCard key={`offer-${item.id}`} item={item} add={add} />)}</div></section>}
    ...
    <section className="storefront-catalogue" id="catalogue">
      ...
      <div className="storefront-tools"><label><span>Rechercher</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Montre, wallet, référence…" /></label><label><span>Catégorie</span><select value={category} onChange={(event) => setCategory(event.target.value)}><option value="Tous">Toutes les catégories</option>{catalog?.categories.map((item) => <option key={item} value={item}>{item}</option>)}</select></label></div>
      {loading && <div className="storefront-state">Chargement du catalogue…</div>}
      {error && <div className="storefront-state error">{error}</div>}
      {!loading && !error && <div className="storefront-product-grid">{filtered.map((item) => <StoreItemCard key={`${item.kind}-${item.id}`} item={item} add={add} />)}{!filtered.length && <div className="storefront-state">Aucun article ne correspond à votre recherche.</div>}</div>}
    </section>
    ...
    {confirmation && <div className="storefront-overlay"><section className="storefront-success" role="dialog" aria-modal="true"><span>Commande reçue</span><h2>Merci pour votre commande.</h2><p>Notre équipe va vous contacter pour confirmer vos informations avant l’expédition.</p><div><small>Référence</small><strong>{confirmation.orderRef || brand}</strong><small>Total produits</small><strong>{money(confirmation.total)}</strong></div>{waDigits && <a href={`https://wa.me/${waDigits}?text=${encodeURIComponent(`Bonjour ${brand}, je viens de passer la commande ${confirmation.orderRef}.`)}`} target="_blank" rel="noreferrer">Contacter {brand}</a>}<button type="button" onClick={() => setConfirmation(null)}>Continuer mes achats</button></section></div>}
  </main>;
}

function StoreItemCard({ item, add }: { item: CatalogItem; add: (item: CatalogItem) => void }) {
  return <article className={`storefront-product ${item.kind === "offer" ? "storefront-offer" : ""} ${!item.available ? "unavailable" : ""}`}>
    <div className="storefront-product-visual">{item.images[0] ? <>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={item.images[0]} alt={item.name} loading="lazy" /></> : <><span>{item.category.slice(0, 1).toUpperCase()}</span><small>{item.category}</small></>}{item.badge && <em className="storefront-product-badge">{item.badge}</em>}{item.lowStock && !item.badge && <em>Dernières pièces</em>}{!item.available && <i>Rupture de stock</i>}</div>
